...
interface MeasurementGuideModalProps {
  ...
  handleMeasurementChange: (measurementId: string, value: string) => void;
}
...
const MeasurementGuideModal = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] bg-[#EEEEEE] max-w-[0vw]! px-[50px] overflow-y-auto">
          ...
          {measurements.map((m, idx) => (
            <div key={m.id}>
              <div>
                ...
                <p className="text-[15px] text-black mt-8">{m.title}</p>
